    ...
    <div>
      <ul>
        {tasksToDoData.map((task) => (
          <div
            className="rounded-xl p-5 m-3"
            style={{ backgroundColor: `${getCardColor(taskType)}` }}
          >
            <p class="text-[#E9F6FF] font-bold text-lg pb-2 uppercase">
              {task.title}
            </p>
            <p class="text-white pb-2">{task.description}</p>
            <div class="flex flex-wrap gap-2 pb-2">
              <Badge color={getStatusColor(task.task_status)}>
                {task.task_status}
              </Badge>
            ...
            <div class="flex flex-wrap gap-2 pb-2">
              {taskType !== "Done" && (
                <>
                  <Badge color="indigo">
                    Start at :{" "}
                    {format(new Date(task.creation_date), "dd/MM/yyyy")}
                  </Badge>
                  <Badge color="indigo">
                  ...
                  <Badge color="pink">
                    you have :{" "}
                    {differenceInDays(task.creation_date, task.deadline)} days
                    left
                  </Badge>
                </>
              )}
              {taskType === "Done" && (
                <Badge color="success">
                  Finiched at : {format(new Date(task.deadline), "dd/MM/yyyy")}
                </Badge>
              )}
            </div>
            ...
            <div class="w-full flex justify-end pt-2">
              {taskType === "To Do" && (
                <button onClick={() => handleChangeStatus(task._id)}>
                  <svg
                    class={`w-[30px] h-[30px] text-[#FFFFFF] dark:text-white`}
                    aria-hidden="true"
              ...
              <button>
                <svg
                  class="w-[30px] h-[30px] text-white dark:text-white"
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentColor"
                  viewBox="0 0 24 24"
